...
class RazorpayProvider(PaymentProviderBase):
    ...
    def __init__(self, config: Dict[str, Any], mock_mode: bool = False):
        ...
        if not mock_mode and not self.key_id:
            logger.warning("Razorpay API keys not configured. Provider will operate in mock mode.")
            self.mock_mode = True
        
        # Initialize Razorpay SDK in production mode
    ...
    async def create_payment_intent(
        ...
        try:
            # Real Razorpay API call (creates Order)
            order_data = {
    ...
    async def cancel_payment_intent(self, provider_intent_id: str) -> bool:
        """Cancel Razorpay order (not supported by Razorpay API)"""
        logger.warning("Razorpay does not support order cancellation via API")
        return False
